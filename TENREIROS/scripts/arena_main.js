...
(function()
{
      ...
      GameHandler.bitmaps.execute(
         function(percentComplete) {
            ...
            if (firstCallback)
            {
               ...
               Game.fillText(ctx, "Cargando...", "14pt Courier New", offx, offy-6, "lightgrey");
               firstCallback = false;
            }
         },
         function() {
   ...
   extend(Arena.AttractorScene, Game.Scene,
   {
      ...
      sceneRendererWelcome: function sceneRendererWelcome(ctx)
      {
         ...
         Game.centerFillText(ctx, "Arena", ff, GameHandler.height*0.5 - 24, "white");
         ...
         Game.centerFillText(ctx, "CLICK para Comenzar", ff, GameHandler.height*0.5 + 12, colour);
         
         ff = ~~(10 * this.world.scale * 2) + "pt Arial";
         Game.fillText(ctx, "by Kevin Roast and Adapted by Ramon Tenreiro", ff, 10, GameHandler.height-(10 * this.world.scale * 2), "white");
      },
      ...
      sceneRendererInfo: function sceneRendererInfo(ctx)
      {
         ...
         ctx.fillText("Como jugar...", GameHandler.width * 0.2, ypos);
         ctx.font = ~~(12 * this.world.scale * 2) + "pt Arial";
         ypos += ~~(32 * this.world.scale * 2);
         ctx.fillText("Use el WASD o el cursor para mover la nave.", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("Use el raton para apuntar las armas.", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("Destruye al enemigo antes de que te alcanze!", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText(" ", GameHandler.width * 0.15, ypos);
      },
...
(function()
{
   Arena.GameOverScene = function(game)
   {
      ...
      var interval = new Game.Interval("SE ACABO", this.intervalRenderer);
   ...
   extend(Arena.GameOverScene, Game.Scene,
   {
      ...
      intervalRenderer: function intervalRenderer(interval, ctx)
      {
            ...
            if (this.game.lastscore === this.game.highscore)
            {
               ...
               try
               {
                  if ($)
                  {
                     ...
                     $("#results").html("<p>Mayor Puntuacion: " + score + "</p>");
         ...
         if (interval.framecounter < 200)
         {
            ...
            if (this.game.lastscore === this.game.highscore)
            {
               Game.centerFillText(ctx, "Nuevo Record!", Game.fontFamily(this.world, 14), GameHandler.height*0.5 + 64, "white");
            }
         }
         else
         {
...
(function()
{
   Arena.GameScene = function(game)
   {
      ...
      this.waves = [
         {
            enemyMax: 3,
            enemyWeighting: [0,0,0,1,1,1,0],
            lifetime: 20
         },
         {
            enemyMax: 3,
            enemyWeighting: [0,1,1,1,0,0],
            lifetime: 20
         },
         {
            enemyMax: 4,
      ...
      var interval = new Game.Interval("JUEGA EN LA ARENA!", this.intervalRenderer);
   ...
   extend(Arena.GameScene, Arena.BaseGameScene,
   {
      ...
      generateMultiplier: function generateMultiplier(enemy)
      {
         if (enemy.dropsMutliplier)
         {
			...
			var count = randomInt(1, (enemy.type < 3 ? enemy.type : 2));
