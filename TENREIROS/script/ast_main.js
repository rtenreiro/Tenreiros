...
function onloadHandler()
{
   ...
   // Load sounds
   if (GameHandler.webkitAudioContext)
   {
   ...
   extend(Asteroids.AttractorScene, Game.Scene,
   {
      ...
      onRenderScene: function onRenderScene(ctx)
      {
         if (this.imagesLoaded)
         {
            ...
            this.sineText(ctx, "ASTEROIDE", GameHandler.width*0.5 - 130, GameHandler.height*0.5 - 64);
         }
         else
         {
		   ...
		   t(ctx, "POR FAVOR ESPERA... CARGANDO IMAGENES...", "18pt Courier New", GameHandler.height*0.5, "white");
         }
      },
      ...
      sceneRendererWelcome: function sceneRendererWelcome(ctx)
      {
		...
		t(ctx, "PULSE BARRA ESPACIADORA o click  PARA EMPEZAR", "14pt Courier New", GameHandler.height*0.5);
         t = (BITMAPS ? Game.fillText : Game.drawText);
         t(ctx, "by Kevin Roast and adapted by Ramon Tenreiro", "8pt Courier New", 16, 624);
      },
      ...
      sceneRendererInfo: function sceneRendererInfo(ctx)
      {
       ...
       /*  t(ctx, "How to play...", "14pt Courier New", 40, 320);
		 ...
		 t(ctx, "Flechas para girar, acelerar y escudo. Barra espaciadora para disparar.", "10pt Courier New", 40, 350);
         t(ctx, "Recoge el disco brillante para mejorar la nave.", "10pt Courier New", 40, 370);
         t(ctx, "¡Cuidado con las naves enemigas!", "10pt Courier New", 40, 390);
         t(ctx, "Presiona S para  poner o quitar el sonido.", "10pt Courier New", 40, 410);
         t(ctx, "Presiona R para cambiar entre moderno y antiguo.", "10pt Courier New", 40, 430);
      },
      ...
      sceneRendererScores: function sceneRendererScores(ctx)
      {
         ctx.fillStyle = ctx.strokeStyle = "white";
         var t = (BITMAPS ? Game.centerFillText : Game.centerDrawText);
         t(ctx, "Puntuacion mas alta", "16pt Courier New", 320);
         ...
         t(ctx, sscore, "16pt Courier New", 350);
      },
   ...
   extend(Asteroids.GameOverScene, Game.Scene,
   {
      ...
      intervalRenderer: function intervalRenderer(interval, ctx)
      {
         ...
         if (interval.framecounter < 300)
         {
            ...
            Game.fillText(ctx, "Puntuacion: " + this.game.score, "14pt Courier New", GameHandler.width*0.5 - 64, GameHandler.height*0.5, "white");
            if (this.game.score === this.game.highscore)
            {
				//Game.fillText(ctx, "New High Score!", "14pt Courier New", GameHandler.width*0.5 - 64, GameHandler.height*0.5 + 24, "white");
               Game.fillText(ctx, "Nuevo Record!", "14pt Courier New", GameHandler.width*0.5 - 64, GameHandler.height*0.5 + 24, "white");
            }
         }
         else
         {
...
(function()
{
   Asteroids.GameScene = function(game, wave)
   {
      ...
      var interval = new Game.Interval("Fase " + wave, this.intervalRenderer);
   ...
   extend(Asteroids.GameScene, Game.Scene,
   {
      ...
      onBeforeRenderScene: function onBeforeRenderScene()
      {
         ...
         if (this.input.left)
         {
            ...
            this.player.heading -= 2 * GameHandler.frameMultipler;//4
         }
         if (this.input.right)
         {
            // rotate clockwise
         ...
         if (this.enemyShipCount <= (this.wave < 2 ? 0 : 1) &&
             GameHandler.frameStart - this.enemyShipAdded > (20000 - (this.wave * 1024)))
         {
            this.enemies.push(new Asteroids.EnemyShip(this, (this.wave < 2 ? 0 : randomInt(0, 1))));
            this.enemyShipCount++;
            this.enemyShipAdded = GameHandler.frameStart;
         }
      ...
      onRenderScene: function onRenderScene(ctx)
      {
         ...
         else
         {
            ...
            if (GameHandler.frameStart - this.player.killedOn > 2000)
            {
   ...
   //var B2  = document.getElementById("B2");
	 // B2.onclick=function(){this.input.left = true;};
  ...
  onKeyDownHandler: function onKeyDownHandler(keyCode)
      ...
      generateAsteroid: function generateAsteroid(speedFactor)
      {
         while (true)
         {
            ...
            {
               var vec = new Vector( ((Rnd()*2)-1)*speedFactor/2, ((Rnd()*2)-1)*speedFactor/2 );
      ...
      generateBabyAsteroids: function generateBabyAsteroids(asteroid, parentVector)
      {
         ...
         if (asteroid.size > 1)
         {
            for (var x=0, xc=randomInt(asteroid.size / 2, asteroid.size - 1); x<xc; x++)
            {
               ...
               //this.enemies.push(baby);//para no generarlos
            }
         }
      },
